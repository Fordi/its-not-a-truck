const getState = () => [...document.querySelectorAll('test-tube')].map(e => (e.getAttribute('contents') || '').split(';').filter(a => a).map(a => a.trim()));
...
const gridToCanonicalString = grid => grid.map(tube => tube.join(';')).sort().join('|');
const solveGrid = (grid, answer = [], visitedPositions = new Set(), tubeHeight = undefined) => {
    if (tubeHeight == undefined) {
        tubeHeight = profileGrid(grid).tubeHeight;
    }
    ...
    for (let i = 0; i < grid.length; i++) {
        ...
        for (let j = 0; j < grid.length; j++) {
            ...
            if (isMoveValid(tubeHeight, tube, candidateTube)) {
                ...
                let firstCount = fromCopy.findIndex(a => a !== firstColor);
                if (firstCount === -1) firstCount = fromCopy.length;                  
                grid2[j].push(...grid2[i].splice(grid2[i].length - firstCount, firstCount));
