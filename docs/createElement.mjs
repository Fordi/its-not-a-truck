...
const isTagName = (n) =>
  ...
  !/^[Xx][Mm][Ll]/.test(n);
const isNsTagName = (t) =>
  isArray(t) && t.length === 2 && typeof t[0] === "string" && isTagName(t[1]);
const isDomRep = (d) =>
  isArray(d) &&
  (typeof d[0] === "function" || isTagName(d[0]) || isNsTagName(d[0]));
const normalizeToDOM = (d) => {
  ...
  throw Object.assign(new Error("Attempted to normalize invalid DOM object:"), {
    detail: d,
  });
};
...
const getPropHandler = (name) => {
  const special = propertyHandlers.find(
    ([matcher]) =>
...
const createElement = (type, props = {}, children = []) => {
  if (type instanceof Function) {
    return normalizeToDOM(
      type(Object.assign(Object.assign({}, props), { children }))
    );
  }
